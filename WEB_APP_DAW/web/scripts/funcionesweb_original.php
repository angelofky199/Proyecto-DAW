...
if(isset($ezg) && ($ezg==0 || $ezg==5 || $ezg==8)){
...
if($dato){return $dato;//-----devolvemos la salida
unset($dato);
mysql_close();
}else return false;
}//------------------fin lee sentencia
...
if($res){$dato=mysql_fetch_assoc($res);
unset($res);
return $dato;//-----devolvemos los datos en un array asociativo
unset($dato);
}}//------------------fin lee sentencia

function introduce_articulo($articulo, $opcion){//------introduce un articulo en el carrito, luego aumenta en 1 el numero de articulos
$esta=false;
$clave=0;
if(isset($opcion)){
...
$_SESSION['linped'][$clave]['unidades']=($_SESSION['linped'][$clave]['unidades'] + 1 );
	...
	if(isset($opcion)){
		...
		}else{ $_SESSION['linped'][$_SESSION['pedido']['numart']]['imagen']=$art['imagen'];
...
function modifica_unidades($art,$uds,$opc){//--------funcion updatea, cambia las unidades de un articulo existente
if(existe($opc)){
...
function calculaimporte(){//----recalcula el importe total tras una insercion, borrado o actualizacion y los gastos de envío
...
}elseif( $_SESSION['pedido']['envio_zona_gastos'] == 8 ){
	if($_SESSION['pedido']['importe']<100){
		$_SESSION['pedido']['envio']=8;
...
function mete_datos(){//-----------------------mete los datos del formulario en pedido
...
function terminapedido(){//-----------------realiza el pedido
...
function insertapedido(){//----inserta el formulario en la BD
mysql_connect("localhost", "admin_essenz", "3S53nc14") or die("No se ha podido conectar: " . mysql_error()); 
mysql_select_db("admin_essenz");
$fecha=date("Y-m-d");
...
function valida(){//------valida el formulario
$mal=false;
$_SESSION["error"]='';
if (!$_SESSION['pedido']['nombre'])  {$mal=true; $_SESSION["error"]['nombre']=true;}
if (!$_SESSION['pedido']['apellidos']) {$mal=true; $_SESSION["error"]['apellidos']=true;}
if (!$_SESSION['pedido']['direccion']) {$mal=true; $_SESSION["error"]['direccion']=true;}
if (!$_SESSION['pedido']['cpostal']) {$mal=true; $_SESSION["error"]['cpostal']=true;}
if (!$_SESSION['pedido']['poblacion']) {$mal=true; $_SESSION["error"]['poblacion']=true;}
if (!$_SESSION['pedido']['telefono']) {$mal=true; $_SESSION["error"]['telefono']=true;}
if (!$_SESSION['pedido']['email']) {$mal=true; $_SESSION["error"]['email']=true;}
if (!$_SESSION['pedido']['provincia']) {$mal=true; $_SESSION["error"]['provincia']=true;}
if (!$_SESSION['pedido']['pais']) {$mal=true; $_SESSION["error"]['pais']=true;}
if (!isset($_SESSION['pedido']['clausula'])) {$mal=true; $_SESSION["error"]['clausula']=true;}
if(($_SESSION['pedido']['pagado'])&&($_SESSION['pedido']['pagado']=='ko')){$mal=true; $_SESSION["error"]['pagado']=true;}
return $mal;
}//------------fin valida
